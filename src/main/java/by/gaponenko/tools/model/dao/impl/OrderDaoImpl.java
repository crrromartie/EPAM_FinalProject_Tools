...
public class OrderDaoImpl implements OrderDao {
    private static final String FIND_ORDER_BY_ID = "SELECT users.user_id, users.login, users.name, " +
            ...
            "tools.is_available, tools.rent_price, tools.photo, orders.order_id, orders.order_date, orders.return_date, " +
            "orders.total_cost, orders.order_status_id FROM orders JOIN users ON users.user_id = orders.user_id " +
            "JOIN tools ON tools.tool_id = orders.tool_id WHERE orders.order_id = ?";
    private static final String FIND_ALL = "SELECT users.user_id, users.login, users.name, " +
            "users.surname, users.email, users.phone, users.user_role_id, users.user_status_id, users.avatar, " +
            "tools.tool_id, tools.tool_type_id, tools.model, tools.description_eng, tools.description_rus, " +
            "tools.is_available, tools.rent_price, tools.photo, orders.order_id, orders.order_date, orders.return_date, " +
            "orders.total_cost, orders.order_status_id FROM orders JOIN users ON users.user_id = orders.user_id " +
            "JOIN tools ON tools.tool_id = orders.tool_id";
    ...
    private static final String FIND_ORDERS_BY_STATUS = "SELECT users.user_id, users.login, users.name, " +
            ...
            "tools.is_available, tools.rent_price, tools.photo, orders.order_id, orders.order_date, orders.return_date, " +
            "orders.total_cost, orders.order_status_id FROM orders JOIN users ON users.user_id = orders.user_id " +
            "JOIN tools ON tools.tool_id = orders.tool_id WHERE orders.order_status_id = ?";
    private static final String FIND_ORDERS_BY_USER_ID = "SELECT users.user_id, users.login, users.name, " +
            "users.surname, users.email, users.phone, users.user_role_id, users.user_status_id, users.avatar, " +
            "tools.tool_id, tools.tool_type_id, tools.model, tools.description_eng, tools.description_rus, " +
            "tools.is_available, tools.rent_price, tools.photo, orders.order_id, orders.order_date, orders.return_date, " +
            "orders.total_cost, orders.order_status_id FROM orders JOIN users ON users.user_id = orders.user_id " +
            "JOIN tools ON tools.tool_id = orders.tool_id WHERE users.user_id = ?";
    private static final String FIND_ORDERS_BY_USER_ID_AND_ORDER_STATUS = "SELECT users.user_id, users.login, users.name, " +
            "users.surname, users.email, users.phone, users.user_role_id, users.user_status_id, users.avatar, " +
            "tools.tool_id, tools.tool_type_id, tools.model, tools.description_eng, tools.description_rus, " +
            "tools.is_available, tools.rent_price, tools.photo, orders.order_id, orders.order_date, orders.return_date, " +
            "orders.total_cost, orders.order_status_id FROM orders JOIN users ON users.user_id = orders.user_id " +
            "JOIN tools ON tools.tool_id = orders.tool_id WHERE users.user_id = ? AND orders.order_status_id = ?";
    ...
    public List<Order> findAllByUserIdAndOrderStatus(long id, Order.Status status) throws DaoException {
        ...
        ResultSet resultSet = null;
        try (Connection connection = ConnectionPool.INSTANCE.getConnection();
             PreparedStatement statement = connection.prepareStatement(FIND_ORDERS_BY_USER_ID_AND_ORDER_STATUS)) {
            statement.setLong(1, id);
